...
## Workflow
1) Read `.agents/CLAUDE.md` and `.agents/plans/IMMUTABLE_BASE_PLAN.md` before changes.
2) Create or update a task in `.agents/tasks/` with clear acceptance criteria.
3) Make changes incrementally; re-check critical flows (modal open/close, popup response).
4) Add a review note in `.agents/reviews/` if requested.
